...
export default function Footer({label,fetchData,songPath}) {
  ...
  useEffect(() =>{
    ...
    function nextSong(){
        ...
        let songName = randomSongPath.replace(`/songs/${label}/`,'').replace('.mp3','')
        const thubnailarr = musicdata?.thumbnail;
        const thumbnail = thubnailarr[random];
        const artistarr = musicdata?.artist;
    ...
    useEffect(() =>{
      if(songPath){
        setSrc(songPath);
        let songName = songPath.replace(`/songs/${label}/`,'').replace('.mp3','')
        setSongName(songName)
  ...
  return (
    <div className='footer'>
        ...
        <Slider aria-label="Volume" value={progress} onChange={handleSeeker}  className='footer-music-slider' />
        </div>
